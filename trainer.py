...
class UAVSegmentationTrainer:
  ...
  def step(self, batch, train=True):
    ...
    if self.model_name == 'deeplab_v3':
      batch_metrics['accuracy'] = (outputs['logits'].argmax(dim=1).view(-1) == labels.view(-1)).sum().item() / len(labels.view(-1))
    else:
  ...
  def save_checkpoint(self, epoch: int, state: dict, name: str, metric: bool = False):
      checkpoint_dir = f"{self.model_name}_metrics" if metric else f"{self.model_name}_checkpoints"
